...
from muagent.connector.schema import Message, Role, PromptField, ChainConfig
...
class AgentFlow:
    ...
    def build_config(self, embeddings: Embeddings = None, llm: BaseLLM = None):
        self.llm_config = self.llm_config if self.llm_config else LLMConfig(model_name="test", llm=self.llm or llm)
        self.embed_config = self.embed_config if self.embed_config else EmbedConfig(embed_model="test", langchain_embeddings=self.embeddings or embeddings)
    ...
    def build_agent(self, 
        ...
        #         pass
        # agent_module = importlib.import_module("muagent.connector.agents")
...
class ChainFlow:
    ...
    def build_chain(self, 
        ...
        # chain 实例化
        chain_module = importlib.import_module("muagent.connector.chains")
...
class PhaseFlow:
    ...
    def __call__(self, params: dict) -> str:
        ...
        try:
            ...
            query = Message(
                role_name="human", role_type="user", tools=self.tools, input_query=query_content,
