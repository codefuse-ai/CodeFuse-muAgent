...
def parse_section_to_dict(text, section_name):
    # Define a pattern to extract the named section along with its content
    section_pattern = rf'#### {section_name}\n(.*?)(?=####|$)'
    ...
    if section_content:
        ...
        # Define a pattern to find segments that follow the format **Key:** Value
        segments_pattern = r'\*\*([^*]+):\*\*\s*(.*?)(?=\*\*|$)'
        
        # Use findall method to extract all matches in the section content
        segments = re.findall(segments_pattern, content, re.DOTALL)
        
        # Convert list of tuples into a dictionary
        section_dict = {key.strip(): value.strip() for key, value in segments}
        
        return section_dict
    else:
        # If the section is not found, return an empty dictionary
        return {}
