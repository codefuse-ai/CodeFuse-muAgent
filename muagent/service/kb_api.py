...
def upload_files2kb(files: List[str], knowledge_base_name: str, embed_config: EmbedConfig, kb_root_path: str= KB_ROOT_PATH):

    # if embedding init failed, you can use override = True
    data = [{"override": True, "file": f, 
            "knowledge_base_name": knowledge_base_name, "not_refresh_vs_cache": False,
            "kb_root_path": kb_root_path, "embed_config": embed_config
            } 
            for f in files]

    for k in data:
        file = Path(k["file"]).absolute().open("rb")
        filename = file.name
